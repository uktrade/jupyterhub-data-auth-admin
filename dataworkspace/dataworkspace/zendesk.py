...
def build_ticket_description_text(dataset_name, dataset_url, contact_email, user, goal_text):
...
def create_zendesk_ticket(
    ...
    client = Zenpy(subdomain=settings.ZENDESK_SUBDOMAIN, email=settings.ZENDESK_EMAIL, token=settings.ZENDESK_TOKEN,)

    ticket_description = build_ticket_description_text(dataset_name, dataset_url, contact_email, user, goal)
    ...
    ticket_audit = client.tickets.create(
        Ticket(
            ...
            custom_fields=[CustomField(id=zendesk_service_field_id, value=zendesk_service_field_value)],
        )
    )
