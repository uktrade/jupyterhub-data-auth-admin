...
class TestAPIDatasetView(TestCase):
    ...
    def test_data(self):
        ...
        source_table = SourceTable.objects.get_or_create(dataset=dataset, database=database, table=table)[0]

        # create external source table
        with psycopg2.connect(database_dsn(settings.DATABASES_DATA[memorable_name])) as conn, conn.cursor() as cur:
            sql = '''
        ...
        expected = {
            ...
            'next': None,
            'values': [[0, 'abigail', '2019-01-01 01:00:00'], [1, 'romeo', '2019-01-01 02:00:00'],],
        }
...
class TestAPIReferenceDatasetView(TestCase):
    ...
    def test_get_data_linked_reference_table(self):
        ...
        linked_rds = factories.ReferenceDatasetFactory.create(group=group, table_name='test_get_ref_data_linked')
        ...
        field3 = factories.ReferenceDatasetFieldFactory.create(
            reference_dataset=rds, name='linked', data_type=8, linked_reference_dataset=linked_rds,
        )
        ...
        self.assertEqual(
            ...
            {
                'headers': ['auto id', 'auto uuid', 'id', 'linked: id', 'linked: name', 'name',],
                'values': [
                    [1, str(rec1.auto_uuid), 1, 1, 'Linked Display Name', 'Test record',],
    ...
    def test_get_data_empty_table(self):
        group = factories.DataGroupingFactory.create()
        rds = factories.ReferenceDatasetFactory.create(group=group, table_name='test_get_empty_ref_data')
        factories.ReferenceDatasetFieldFactory.create(reference_dataset=rds, name='id', data_type=2, is_identifier=True)
        factories.ReferenceDatasetFieldFactory.create(reference_dataset=rds, name='name', data_type=1)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            json.loads(output.decode('utf-8')), {'headers': ['id', 'name'], 'values': [], 'next': None},
        )

    def test_get_data_search_after(self):
        group = factories.DataGroupingFactory.create()
        rds = factories.ReferenceDatasetFactory.create(group=group, table_name='test_get_ref_data_search_after')
        field1 = factories.ReferenceDatasetFieldFactory.create(
            reference_dataset=rds, name='id', data_type=2, is_identifier=True
        )
        field2 = factories.ReferenceDatasetFieldFactory.create(reference_dataset=rds, name='name', data_type=1)
        rds.save_record(
            None, {'reference_dataset': rds, field1.column_name: 1, field2.column_name: 'Test record',},
        )
        rds.save_record(
            None, {'reference_dataset': rds, field1.column_name: 2, field2.column_name: 'Ánd again',},
        )
        ...
        self.assertEqual(
            json.loads(output.decode('utf-8')), {'headers': ['id', 'name'], 'values': [[2, 'Ánd again']], 'next': None},
        )
