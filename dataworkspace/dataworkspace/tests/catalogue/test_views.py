...
def test_old_dataset_url_redirects_to_new_url(client):
    ...
    response = client.get(
        reverse('catalogue:dataset_fullpath', kwargs={'group_slug': ds.grouping.slug, 'set_slug': ds.slug},)
    )
    assert response.status_code == 302
...
def test_old_reference_dataset_url_redirects_to_new_url(client):
    ...
    response = client.get(
        reverse('catalogue:reference_dataset', kwargs={'group_slug': rds.group.slug, 'reference_slug': rds.slug},)
    )
    assert response.status_code == 302
