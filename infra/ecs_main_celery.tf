resource "aws_ecs_service" "celery" {
  ...
  service_registries {
    registry_arn   = "${aws_service_discovery_service.celery.arn}"
  }
...
resource "aws_service_discovery_service" "celery" {
  name = "${var.prefix}-celery"
  dns_config {
...
data "template_file" "celery_container_definitions" {
  ...
  vars {
    container_image   = "${var.admin_container_image}:${data.external.celery_current_tag.result.tag}"
    container_name    = "${local.celery_container_name}"
    container_command = "[\"/dataworkspace/start-celery.sh\"]"
    ...
    secret_key                = "${random_string.celery_secret_key.result}"
...
data "external" "celery_current_tag" {
  program = ["${path.module}/container-tag.sh"]

  query = {
    cluster_name = "${aws_ecs_cluster.main_cluster.name}"
    service_name = "${var.prefix}-celery"  # Manually specified to avoid a cycle
    container_name = "jupyterhub-celery"
  }
}
...
data "template_file" "celery_store_db_creds_in_s3_container_definitions" {
  template = "${file("${path.module}/ecs_main_admin_container_definitions.json")}"
...
resource "random_string" "celery_secret_key" {
  length = 256
  special = false
}
...
resource "aws_cloudwatch_log_subscription_filter" "celery" {
  count = "${var.cloudwatch_subscription_filter ? 1 : 0}"
  name            = "${var.prefix}-cekery"
...
data "aws_iam_policy_document" "admin_run_tasks" {
  statement {
    ...
    resources = [
      ...
      "arn:aws:ecs:${data.aws_region.aws_region.name}:${data.aws_caller_identity.aws_caller_identity.account_id}:task-definition/${aws_ecs_task_definition.superset.family}-*",
      "arn:aws:ecs:${data.aws_region.aws_region.name}:${data.aws_caller_identity.aws_caller_identity.account_id}:task-definition/${aws_ecs_task_definition.dataexplorer.family}",
      "arn:aws:ecs:${data.aws_region.aws_region.name}:${data.aws_caller_identity.aws_caller_identity.account_id}:task-definition/${aws_ecs_task_definition.dataexplorer.family}-*",
...
resource "aws_iam_role_policy_attachment" "celery_store_db_creds_in_s3_task" {
  role       = "${aws_iam_role.celery_store_db_creds_in_s3_task.name}"
  policy_arn = "${aws_iam_policy.celery_store_db_creds_in_s3_task.arn}"
}
...
data "aws_iam_policy_document" "celery_store_db_creds_in_s3_task" {
  statement {
    actions = [
        "s3:PutObject",
        "s3:PutObjectAcl",
